...
const fiveAmCronJob = () => {
  ...
  const queryString = `
    SELECT user.first_name, user.last_name, user.email, user.phone, user.subscription_plan, location_to_track.latitude, location_to_track.longitude from user 
    ...
    ON user.tracking_id IN (
      SELECT location_to_track.tracking_id from location_to_track WHERE location_to_track.location_id IN (
          SELECT time_to_track.location_id FROM time_to_track WHERE time_to_track.time = '9:00 am'
      )
  )
  `;
...
const sendNotification = async ({
  ...
  if (subscription_plan === "free") {
    ...
    const weatherUpdateTextContent = `
      Hey ${first_name} ${last_name}, here is your weather update for ${location_name}.
    ...
    const weatherHTMLContent = `
      <html>
          <body>
              <h3 style={{color: 'red'}}>
                  Hey ${first_name}, here is your weather update for ${location_name}.
              </h3>
